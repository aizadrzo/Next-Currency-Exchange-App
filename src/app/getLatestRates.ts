...
export const getLatestRates = async (base: keyof typeof Currencies = "MYR"): Promise<Currency[]> => {
  ...
  const data = Object.entries(latestRates).map(([currency, rate]) => {
    const previousRate = yesterdayRate[currency as keyof typeof Currencies];
    const change = previousRate ? rate - previousRate : 0;
    const changePercentage = previousRate
      ? ((change / previousRate) * 100).toFixed(2)
      : "N/A";
    const formatRates = new Intl.NumberFormat(Currencies[currency as keyof typeof Currencies].locale, {
      style: "currency",
      currency: currency,
    }).format(rate)

    return {
      baseCurrency: baseCurrency as keyof typeof Currencies,
